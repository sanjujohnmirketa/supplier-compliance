...
export default class ScSupplierPortal extends NavigationMixin(LightningElement) {
    ...
    _buildComplianceRow(x, i) {
        ...
        // Summary content only exists for the supplier to see once Procurement
        // (or the Analyst, whichever currently owns the case) has approved it —
        // approved mirrors Compliance_Assessment__c.Analyst_Validated__c.
        // Un-approved rows show upload/verdict status only, no reasoning detail.
        ...
        return {
            ...
            // WFL-03/04: a rejected requirement gets its own Re-upload (targets
            // this exact assessmentId, same mechanism as handleRenewClick) and
            // Add comment actions in place of the old generic uploader. Clears
            // the moment the supplier responds — linkDocumentToCompliance/
            // DocumentProcessingController already null out
            // Procurement_Decision__c on re-upload, so a fresh upload takes the
            // row out of this state on its own without any extra client logic.
            isRejected: x.procurementDecision === 'Rejected',
    ...
    // ── WFL-03/04: respond to a rejection — Re-upload or Add comment ──────────
    // Re-upload reuses handleRenewClick/handleRenewFilePicked as-is: that
    // mechanism already re-targets an exact assessmentId instead of relying
    // on the fuzzy-matcher, which is exactly what a rejection response needs
    // too (see the handleRenewClick doc comment for why the fuzzy path can't
    // be used here — a rejected row is still "ticked", same as an expired one).
    handleReuploadRejected(event) {
        this.handleRenewClick(event);
    }
